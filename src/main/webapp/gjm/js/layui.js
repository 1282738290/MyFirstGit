...
layui.use(["layer", "laytpl", "form"], function() {
	...
	$(".top-page-app").on("click", function() {
		...
		per.open({
			...
			success: function(layero, index) {
				var pindex = per.getFrameIndex(window.name);
				var alldesktopapp = layui.data("desktop-app")["desktop-app-" + pindex];
				if (!alldesktopapp) {
					alldesktopapp = [index]
				} else {
	...
	$(".open-this-app").on("click", function() {
		var url = $(this).attr("href"),
			title = $(this).text(),
			width = "500",
			height = "500";
